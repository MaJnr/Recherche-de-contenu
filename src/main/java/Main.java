...
public class Main extends Application {
    ...
    private void openHelpWindow() {
        ...
        Text editorUseDesc = new Text("Une fois avoir cliqué sur un résultat, l'éditeur de contenu s'ouvre. Il est composé de plusieurs éléments :\n\n" +
                ...
                "La saisie des majuscules des accents et des tirets sont inutile, de même que l'ordre dans lequel les mots sont rentrés. Les mots " +
                "'arc en ciel paysage' auront la même valeur que 'arc paysage ciel en'.\n");
       ...
       /* helpGroup.setOnScroll(new EventHandler<ScrollEvent>() {
        ...
        sb.valueProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                helpVbox.setLayoutY(-newValue.doubleValue() * 2);
            }
        });
...
//        }
    ...
    private void writeInXMLFile(String absolutePathName, String fileName) {
        ...
        for (String imgExt : SUPPORTED_IMAGES_EXTENSIONS) {
            if (!wasFileAddedToList && splitStrings[splitStrings.length - 1].equalsIgnoreCase(imgExt)) {
                ...
                String tagToWrite = "";
                // search for the metadata of the media
                try {
                    Metadata metadata = ImageMetadataReader.readMetadata(new File(absolutePathName));
                    for (Directory d : metadata.getDirectories()) {
                        for (Tag tag : d.getTags()) {
                            if (tag.getTagName().equalsIgnoreCase("File Modified Date")) {
                                tagToWrite = formatFileModifiedDate(tag.getDescription());
                                System.out.println(fileName + "  =====>  " + tag.getDescription());
                            }
                        }
                    }
                    crud.addRecord(_idIncrNumber, "image", fileName, tagToWrite, absolutePathName);
    ...
    // changes the date format to fit the tag of a content
    // could use a date formatter if possible...
    private String formatFileModifiedDate(String description) {
        String outputFormat = "";
        ...
        // the infos, we do't need the time zone nor the precise hour
        String day = words[0];
        String month = words[1];
        String dayNumber = words[2];
        String year = words[5];
        ...
        switch (day) {
            ...
            case "mar.":
                ...
                break;
            case "mer.":
                outputFormat += "mercredi ";
                break;
            case "jeu.":
                outputFormat += "jeudi ";
                break;
            case "ven.":
                outputFormat += "vendredi ";
                break;
            case "sam.":
                outputFormat += "samedi ";
                break;
            case "dim.":
                outputFormat += "dimanche ";
                break;
        }

        outputFormat += dayNumber;

        switch (month) {
            case "janv.":
                outputFormat += " janvier ";
                break;
            case "févr.":
                outputFormat += " février ";
                break;
            case "mars":
                outputFormat += " mars ";
                break;
            ...
            case "juin":
                ...
                break;
            case "juil.":
                outputFormat += " juillet ";
                break;
            case "août":
                outputFormat += " août ";
                break;
            case "sept.":
                outputFormat += " septembre ";
                break;
            case "oct.":
                outputFormat += " octobre ";
                break;
            case "nov.":
                outputFormat += " novembre ";
                break;
            case "déc.":
                outputFormat += " décembre ";
                break;
        }
...
/*
    ...
    private void openEmbedEditor(String[] resultRow) {
        ...
        if (type != null && type.equals("image")) {
            try {
                image = new Image(new FileInputStream(path), embedEditorBox.getPrefWidth() * 0.9, embedEditorBox.getHeight() * 0.7, true, false);
                imageView = new ImageView(image);
               /* imageView.setFitWidth(embedEditorBox.getPrefWidth() * 0.9);
                imageView.setFitHeight(embedEditorBox.getHeight() * 0.7);
                imageView.setPreserveRatio(true);*/
